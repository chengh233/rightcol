...
| **TTM** | Trailing Twelve Months | **最近十二个月** | **把最近四个季度加起来当"一年"用**。每出一份新季报就踢掉最老的一季、加上新的 —— 是一个**滚动的一年**。详见下方 §2.8 |
| **CAGR** | Compound Annual Growth Rate | 复合年化增长率 | |
| **YoY / QoQ** | Year-over-Year / Quarter-over-Quarter | 同比 / 环比 | |

## 2.8 TTM 与季度序列 —— 两个不同的问题

**TTM（Trailing Twelve Months，最近十二个月）= 最近四个季度相加。**

```
2026-05  +  2026-02  +  2025-11  +  2025-08   =  TTM
 41.5B      23.9B       13.6B       11.3B       90.3B
```

**为什么要加成一年，而不直接看季度？**

1. **消除季节性** —— TTM 永远包含每个财季各一个，季节差异自动抵消。
   （很多生意季节性极强：苹果 12 月那季最大，Costco 的第四财季是 16 周而非 12 周。）
2. **比率类指标需要完整年度的分子** —— ROIC 的分子是流量、分母是存量，
   用单季数字要乘 4 年化，而那假设了这一季有代表性。
3. **单季噪音大** —— 发货时点、产品发布、工作日天数都会干扰。
   实测 AMD：2026Q1 环比 −0.2% 像是"增长停了"，下一季环比 **+13%**。
4. **跨公司唯一可比的口径** —— 各家财年不同，"最新财年"跨度可达 368 天。

**但 TTM 有个固有弱点：它滞后。**

TTM 本质是**四季滚动平均**，而平均值按定义就会滞后。实测 Micron：
...
| | 回答什么 | 用在哪 |
|---|---|---|
| **TTM** | **现在的「水平」是多少** | ROIC、估值、**跨公司比较** |
| **季度序列** | **正在往哪走、拐了没有** | 找拐点、看加速减速、判断周期位置 |

> **打个比方：TTM 是体检报告（现在的健康水平，适合和别人比），
> 季度是心电图（刚才发生了什么，适合发现异常）。两个都要看。**
