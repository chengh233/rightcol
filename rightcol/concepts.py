...
# ⚠️ 这些是**期间平均值**，不是流量 —— TTM 时要取平均而非求和，
# 且不能用「全年 − 前三季」倒推 Q4。
#
# 实测事故：苹果的 TTM 摊薄股数被四季相加算成 44.20B（实际约 14.7B，虚增 3 倍），
# 而其中一季还是 −0.05B —— 那是 derive_q4 拿"全年加权平均股数 − 前三季加权平均
# 股数"倒推出来的垃圾值。加权平均股数在数学上根本不能这样相加或相减。
PERIOD_AVERAGE: set[str] = {"shares_diluted", "shares_basic"}
